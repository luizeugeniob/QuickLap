...
public class UserRepositoryTests : InMemoryDatabaseFixture
{
    ...
    {
        // Arrange
        var user = new User { Id = 1, Email = "email@email.com", Password = "123456789" };
        await Context.Users.AddAsync(user);
        await Context.SaveChangesAsync();

        // Act
        await GetRepository().CompleteProfileAsync(1, "NewFirstName", "NewLastName");
    ...
    public async Task CompleteProfileAsync_ShouldThrowNotFoundException_WhenUserDoesNotExist()
    {
        ...
        // Assert
        await GetRepository()
            .Invoking(async x => await x.CompleteProfileAsync(1, "NewFirstName", "NewLastName"))
